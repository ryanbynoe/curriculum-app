pipeline {
  ...
  stages {
    ...
    stage('Log Into DockerHub') {
      ...
      steps {
        powershell '$ErrorActionPreference = "Stop"  try {     if (-not $env:DOCKERHUB_USERNAME -or -not $env:DOCKERHUB_PASSWORD) {         throw "Docker Hub credentials not found in environment variables"     }      $securePassword = ConvertTo-SecureString $env:DOCKERHUB_PASSWORD -AsPlainText -Force     $credential = New-Object System.Management.Automation.PSCredential ($env:DOCKERHUB_USERNAME, $securePassword)      $loginOutput = docker login -u $credential.UserName -p $credential.GetNetworkCredential().Password 2>&1     if ($LASTEXITCODE -ne 0) {         throw "Docker login failed: $loginOutput"     }      Write-Output "Docker login successful"      docker info     if ($LASTEXITCODE -ne 0) {         throw "Failed to retrieve Docker info"     }      Write-Output "Docker connection verified"  } catch {     Write-Error "An error occurred: $_"     exit 1 } finally {     docker logout     Remove-Variable -Name securePassword, credential -ErrorAction SilentlyContinue }'
      }
    }
