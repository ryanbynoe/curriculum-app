pipeline {
  ...
  stages {
    ...
    stage('Log Into DockerHub') {
      ...
      steps {
        powershell '# Docker Login PowerShell Script  # Function to check if a command exists function Test-Command($cmdname) {     return [bool](Get-Command -Name $cmdname -ErrorAction SilentlyContinue) }  # Check if Docker is available if (-not (Test-Command "docker")) {     Write-Host "Error: Docker command not found. Please ensure Docker is installed and in your PATH." -ForegroundColor Red     exit 1 }  # Check if required environment variables are set if (-not $env:DOCKERHUB_USER -or -not $env:DOCKERHUB_PASSWORD) {     Write-Host "Error: DOCKERHUB_USER and/or DOCKERHUB_PASSWORD environment variables are not set." -ForegroundColor Red     exit 1 }  # Attempt Docker login try {     Write-Host "Attempting Docker login for user: $env:DOCKERHUB_USER" -ForegroundColor Cyan     $securePassword = ConvertTo-SecureString $env:DOCKERHUB_PASSWORD -AsPlainText -Force     $credential = New-Object System.Management.Automation.PSCredential ($env:DOCKERHUB_USER, $securePassword)          docker login -u $credential.UserName --password-stdin <# PowerShell automatically converts SecureString to plain text for stdin #> | Out-Null          if ($LASTEXITCODE -eq 0) {         Write-Host "Docker login successful." -ForegroundColor Green     } else {         throw "Docker login failed with exit code $LASTEXITCODE"     } } catch {     Write-Host "Error during Docker login: $_" -ForegroundColor Red     exit 1 } finally {     # Clear sensitive data from memory     Remove-Variable -Name securePassword -ErrorAction SilentlyContinue     Remove-Variable -Name credential -ErrorAction SilentlyContinue }  Write-Host "Docker login script completed." -ForegroundColor Green'
      }
    }

    stage('Push') {
      steps {
        powershell '# Docker Push PowerShell Script  # Function to check if a command exists function Test-Command($cmdname) {     return [bool](Get-Command -Name $cmdname -ErrorAction SilentlyContinue) }  # Check if Docker is available if (-not (Test-Command "docker")) {     Write-Host "Error: Docker command not found. Please ensure Docker is installed and in your PATH." -ForegroundColor Red     exit 1 }  # Define the image name and tag $imageName = "ryanbynoe/curriculum-front:latest"  # Check if the image exists locally $imageExists = docker images -q $imageName if (-not $imageExists) {     Write-Host "Error: Image $imageName not found locally. Make sure you\'ve built the image before pushing." -ForegroundColor Red     exit 1 }  # Attempt to push the Docker image try {     Write-Host "Attempting to push image: $imageName" -ForegroundColor Cyan     docker push $imageName      if ($LASTEXITCODE -eq 0) {         Write-Host "Docker push successful for $imageName" -ForegroundColor Green     } else {         throw "Docker push failed with exit code $LASTEXITCODE"     } } catch {     Write-Host "Error during Docker push: $_" -ForegroundColor Red     exit 1 }  Write-Host "Docker push script completed." -ForegroundColor Green'
      }
    }
